...
export class AdminSystemController {
  ...
  @ApiOperation({
    summary: 'Encerrar posição específica sem lucro/perda',
    description: 'Cria job de venda fictício no preço de entrada para fechar uma posição específica sem lucro ou perda. Tudo é criado diretamente no banco, sem passar pelo executor ou exchange.'
  })
  @ApiResponse({
    status: 200,
  ...
  async closePositionsBreakeven(@Body() dto: {
    ...
    const { position_id } = dto;

    if (!position_id || isNaN(position_id) || position_id <= 0) {
      throw new BadRequestException('position_id é obrigatório e deve ser um número válido maior que 0');
    }
    ...
    if (!position) {
      return {
        ...
        job_created: null,
        execution_created: null,
        error: `Posição #${position_id} não encontrada`,
      };
    }
    ...
    if (position.status !== 'OPEN') {
      return {
        ...
        job_created: null,
        execution_created: null,
        error: `Posição #${position_id} não está aberta (status: ${position.status})`,
        position_info: {
          symbol: position.symbol,
          qty_remaining: position.qty_remaining.toNumber(),
          price_open: position.price_open.toNumber(),
          exchange_account: position.exchange_account.label || `${position.exchange_account.exchange} #${position.exchange_account.id}`,
        },
      };
    }
    ...
    if (qtyRemaining <= 0) {
      return {
        ...
        job_created: null,
        execution_created: null,
        error: `Posição #${position_id} não tem quantidade restante para fechar`,
        position_info: {
          symbol: position.symbol,
          qty_remaining: qtyRemaining,
          price_open: position.price_open.toNumber(),
    ...
    try {
      ...
      return {
        ...
        job_created: result.job_id,
        execution_created: result.execution_id,
        position_info: {
          symbol: position.symbol,
          qty_remaining: qtyRemaining,
          price_open: position.price_open.toNumber(),
      ...
      console.error(`[DEBUG-TOOLS] Erro ao fechar posição #${position_id}:`, error);
      return {
        position_id,
        position_closed: false,
        job_created: null,
        execution_created: null,
        error: `Erro ao fechar posição: ${error.message}`,
        position_info: {
          symbol: position.symbol,
          qty_remaining: qtyRemaining,
          price_open: position.price_open.toNumber(),
