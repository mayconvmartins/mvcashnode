      ...
      try {
                    ...
                    if (doubleCheckJob) {
                      ...
                      try {
                        ...
                        if (!validationResult.valid) {
                          this.logger.warn(`[SL-TP-MONITOR-SIM] ⚠️ Take Profit SKIPADO para posição ${position.id}: ${validationResult.reason}`);
                          await this.prisma.tradePosition.update({
                            where: { id: position.id },
                            data: { tp_triggered: false },
                          });
        ...
        if (position.tsg_enabled && 
          ...
          if (position.tsg_triggered) {
            ...
            if (recentFailedJob) {
              this.logger.warn(
                `[SL-TP-MONITOR-SIM] ✅ BUG-013 FIX: Flag tsg_triggered resetada para posição ${position.id}: ` +
              ...
              if (pnlPct > sellThreshold || pnlPct < minViableThreshold) {
                ...
                this.logger.log(
                  `[SL-TP-MONITOR-SIM] ✅ BUG-013 FIX: Posição ${position.id} com tsg_triggered=true mas sem job ativo, ` +
        ...
        if (position.trailing_enabled && position.trailing_distance_pct) {
          ...
          if (currentPrice <= trailingTriggerPrice && !position.trailing_triggered) {
            ...
            try {
              ...
              if (!validationResult.valid) {
                this.logger.warn(`[SL-TP-MONITOR-SIM] ⚠️ Trailing Stop SKIPADO para posição ${position.id}: ${validationResult.reason}`);
                // Reverter lock
                await this.prisma.tradePosition.update({
                  where: { id: position.id },
                  data: { trailing_triggered: false }
